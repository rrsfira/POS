...
const PurchasePaymentsTable = ({ data }) => {
  ...
  const handleExportPDF = () => {
      ...
      doc.text("Data Sales Payments Supplier Report", 14, yOffset);
      yOffset += 10;
    const tableColumn = ["Date","Reference", "Purchase", "Paid by", "Amount"];
    const tableRows = filtered.map((row) => [
      ...
      doc.save("Sales Payments Supplier Report.pdf");
  };
       ...
       <div className="overflow-x-auto mt-6">
         <table className="min-w-full table-auto border-collapse">
           <thead className="bg-base-100">
             <tr>
               {["Date","Reference", "Purchase", "Paid by", "Amount"].map((header, i) => (
                <th
                  key={i}
...
export default PurchasePaymentsTable;
