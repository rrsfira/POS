...
function AllTransfer() {
  ...
  const navigateToEditPage = () => setIsEditPage(true);
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Transfers Report", 10, 10);
    ...
    doc.autoTable({
      head: [
        ["Date", "Reference", "From Warehouse", "To Warehouse", "Items", "Grand Total", "Status"],
      ],
      body: tableData,
      startY: 20,
    });
  
    doc.save("Transfers_Report.pdf");
  };  
